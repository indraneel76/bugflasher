...
public class HumiditySensor extends Device {
	...
	public int humidity ;
	private static final Logger logger = Logger.getLogger(HumiditySensor.class.getName());
	...
	public Response sendData(@Context HttpServletRequest req, 
		...
		humidity=Integer.parseInt(params);
		doAction();
		String output="{\"status\":\"humidity received\"}" ;
	...
	{
		try {
		String url = "https://api.thingspeak.com/update?api_key=BJUYTDRYM9QBRW01&field1="+humidity;
		logger.info("humidity url = "+url);
		URL obj = new URL(url);
